    ...
    <div class="row">
        ...
        <div class="invoice p-3 mb-3">
            ...
            <div class="col-6">
                ...
                <br>
                <h1 class="text-purple">Pedido Solicitado!</h1>
            ...
            <div class="row no-print">
                <div class="col-12">
                    <a href="invoice-print.html" target="_blank" class="btn btn-default btn-sm"><i class="fas fa-print"></i> Print</a>
                    <a  href="{{route('employee.orders.pagarOrder', $order[0]->slugPedido)}}"  class="btn btn-success float-right btn-sm">Pagar</a>

                    <form action="{{route('employee.orders.aceptarOrder')}}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" value="{{$order[0]->slugPedido}}" name="slug_pedido">
                        <button type="submit" class="btn btn-primary float-right btn-sm" style="margin-right: 5px;">
                        Aceptar
                        </button>
                    </form>

                    <form action="{{route('employee.orders.cancelarOrder')}}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" value="{{$order[0]->slugPedido}}" name="slug_pedido">
                        <button type="submit" class="btn btn-danger float-right btn-sm" style="margin-right: 5px;">
                        Cancelar
                        </button>
                    </form>
